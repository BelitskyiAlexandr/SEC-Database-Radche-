...
public class _11ARepository
{
    private SqliteConnection connection;

    public _11ARepository(SqliteConnection connection)
    {
    ...
    public List<Pupil> GetAll()
    {
        ...
        command.CommandText = @"SELECT * FROM _11A";
    ...
    public Pupil GetById(long id)
    {
        ...
        SqliteCommand command = connection.CreateCommand();
        command.CommandText = @"SELECT * FROM _11A WHERE id = $id";
    ...
    public int DeleteById(long id, MathRepository mathRepository, BiologyRepository biologyRepository)
    {
        ...
        SqliteCommand command = connection.CreateCommand();
        command.CommandText = @"DELETE FROM _11A WHERE id = $id";
        command.Parameters.AddWithValue("$id", id);
    ...
    public bool Update(long id, Pupil pupil, MathRepository mathRepository, BiologyRepository biologyRepository)
    {
        ...
        connection.Open();
        SqliteCommand command = connection.CreateCommand();
        command.CommandText = $"UPDATE _11A SET surname = $surname, name = $name WHERE id = $id";
        command.Parameters.AddWithValue("$id", id);
    ...
    public long Insert(string surname, string name)
    {
       ...
       @" 
        ...
        command.Parameters.AddWithValue("$class", "11A");
