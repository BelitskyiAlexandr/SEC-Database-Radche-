...
public class MathRepository
{
    ...
    public MathRepository(SqliteConnection connection)
    {
    ...
    public List<Subject> GetAll()
    {
        ...
        command.CommandText = @"SELECT * FROM math";
    ...
    public Subject GetSameStudent(Pupil pupil)
    {
        connection.Open();
        SqliteCommand command = connection.CreateCommand();
        command.CommandText = $"SELECT * FROM math WHERE surname = $surname AND name = $name";
        command.Parameters.AddWithValue("$surname", pupil.surname);
        command.Parameters.AddWithValue("$name", pupil.name);

        SqliteDataReader reader = command.ExecuteReader();

        Subject mathPupil = new Subject();
        if (reader.Read())
        {
            mathPupil.id = int.Parse(reader.GetString(0));
            mathPupil.surname = reader.GetString(1);
            mathPupil.name = reader.GetString(2);
            mathPupil.classroom = reader.GetString(3);
            mathPupil.mark = int.Parse(reader.GetString(4));
        }
        reader.Close();
    ...
    public bool Update(long id, Subject subj)
    {
        connection.Open();
        SqliteCommand command = connection.CreateCommand();
        command.CommandText = $"UPDATE math SET surname = $surname, name = $name, class = $class, mark = $mark WHERE id = $id";
        command.Parameters.AddWithValue("$id", id);
    ...
    public int DeleteById(long id)
    {
        ...
        SqliteCommand command = connection.CreateCommand();
        command.CommandText = @"DELETE FROM math WHERE id = $id";
        command.Parameters.AddWithValue("$id", id);
